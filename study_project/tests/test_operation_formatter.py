...
from study_project.src import formatters


@pytest.fixture
def formatter():
    return formatters.OperationFormatter(
        date_format='%d.%m.%Y',
...
@pytest.mark.parametrize(
    'requisites,expected_formatted_requisites',
    [
...
def test_format_requisites(formatter, requisites, expected_formatted_requisites):
    assert formatter.format_requisites(requisites) == expected_formatted_requisites
